...
class NPA(nn.Module):
    ...
    def __init__(self, input_dim=128, hidden_dim=128, attn_dim=256, fc_dim=512,
                 num_layers=1, question_num=123, dropout=0.0):
        ...
        # embedding layers
        self._response_embedding_layer = nn.Embedding(num_embeddings=2*question_num+1,
        ...
        self.classifier = self._fc_layers._fc4
    ...
    def forward(self, x, target_id):
        ...
        # Bi-LSTM layer
        output, _ = self._lstm(x, (hidden[0].detach(), hidden[1].detach()))
