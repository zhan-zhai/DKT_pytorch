...
class DKVMN(nn.Module):
    ...
    def __init__(self, key_dim, value_dim, summary_dim, question_num, concept_num):
        ...
        self._key_memory = torch.Tensor(self._key_dim, self._concept_num).to(device)
    ...
    def _init_value_memory(self):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self._value_memory = torch.Tensor(self._value_dim, self._concept_num).to(device)
    ...
    def _compute_correlation_weight(self, question_id):
        ...
        """
        device = question_id.device
        question_vector = self._question_embedding(question_id).to(device)
        return self._softmax(torch.matmul(question_vector, self._key_memory)).to(device)
